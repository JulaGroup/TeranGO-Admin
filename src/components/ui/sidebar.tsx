...
function useSidebar() {
  ...
  if (!context) {
    // Provide a safe fallback when no SidebarProvider is present.
    // This allows components that optionally render a sidebar trigger
    // to safely mount outside of a layout that uses the provider.
    return {
      state: "expanded",
      open: false,
      setOpen: () => {},
