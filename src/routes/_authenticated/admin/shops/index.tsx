...
const topNav = [
  ...
  { title: "Shops", href: "/admin/shops", isActive: true },
];
...
function ShopsPage() {
  ...
  const filtered = shops.filter((s) => {
    const matchSearch =
      ...
      s.shopType?.toLowerCase().includes(search.toLowerCase()) ||
      s.vendor?.user?.fullName?.toLowerCase().includes(search.toLowerCase());
    const matchStatus =
      filterStatus === "all" ||
  ...
  const stats = {
    ...
    accepting: shops.filter((s) => s.acceptsOrders).length,
    avgRating:
      shops.length > 0
        ? (
            shops.reduce((a, b) => a + (b.rating ?? 0), 0) / shops.length
          ).toFixed(1)
        : "—",
  };
  ...
  const updateMutation = useMutation({
    mutationFn: async (data: typeof editForm) => {
      ...
      return adminApi.updateShop(selected!.id, { ...data, imageUrl });
    },
    onSuccess: () => {
  ...
  const deleteMutation = useMutation({
    mutationFn: () => adminApi.deleteShop(selected!.id),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["admin-shops"] });
  ...
  const toggleStatusMutation = useMutation({
    mutationFn: ({ id, isActive }: { id: string; isActive: boolean }) =>
      adminApi.updateShop(id, { isActive }),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["admin-shops"] });
  ...
  const toggleOrdersMutation = useMutation({
    mutationFn: ({
      ...
      id: string;
      acceptsOrders: boolean;
    }) => adminApi.updateShop(id, { acceptsOrders }),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["admin-shops"] });
            ...
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-2 gap-3 text-sm">
                ...
                <InfoRow
                  icon={Package}
                  label="Products"
                  value={String(selected._count?.products ?? 0)}
                />
                <InfoRow
                  icon={Package}
                  label="Total Orders"
            ...
            <div className="space-y-5 py-2">
              ...
              <div className="space-y-2">
                ...
                <div
                  ...
                  onClick={() =>
                    document.getElementById("shop-img-input")?.click()
                  }
                >
                  {imagePreview ? (
                    <img
              ...
              <div className="grid gap-4 sm:grid-cols-2">
                ...
                <div className="space-y-1">
                  ...
                  <Input
                    ...
                    onChange={(e) =>
                      setEditForm((f) => ({ ...f, shopType: e.target.value }))
                    }
                  />
                </div>
                <div className="space-y-1">
          ...
          <AlertDialogContent>
            <AlertDialogHeader>
              ...
              <AlertDialogDescription>
                ...
                ? This will also delete all associated products. This action
                cannot be undone.
              </AlertDialogDescription>
            </AlertDialogHeader>
            <AlertDialogFooter>
        ...
        <div className="flex items-start justify-between">
          ...
          <DropdownMenu>
            ...
            <DropdownMenuContent align="end">
              ...
              <DropdownMenuItem
                onClick={() => onToggleOrders(s.id, !s.acceptsOrders)}
              >
                {s.acceptsOrders ? (
                  <>
      ...
      <CardContent className="space-y-2 pb-3">
        <div className="flex items-center justify-between text-xs text-muted-foreground">
          <span className="flex items-center gap-1">
            ...
            {s.rating ? s.rating.toFixed(1) : "—"} ({s.totalReviews ?? 0})
          </span>
          <span>{s._count?.orders ?? 0} orders</span>
          <span>{s._count?.products ?? 0} products</span>
        </div>
        {s.vendor?.user?.fullName && (
          <p className="truncate text-xs text-muted-foreground">
            Vendor: {s.vendor.user.fullName}
          </p>
        )}
